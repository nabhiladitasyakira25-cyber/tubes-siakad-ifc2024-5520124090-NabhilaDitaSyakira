        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-stone-200">
                <div class="p-6 text-stone-900">
                    <form action="{{ route('mahasiswa.update', $mahasiswa->npm) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <label class="block text-sm font-medium text-stone-500">NPM (Primary Key tidak dapat diubah)</label>
                            <input type="text" value="{{ $mahasiswa->npm }}" class="mt-1 block w-full rounded-md border-stone-200 bg-stone-50 text-stone-500 font-mono shadow-sm cursor-not-allowed" disabled>
                        </div>

                        <div>
                            <label for="nama" class="block text-sm font-medium text-stone-700">Nama Lengkap</label>
                            <input type="text" name="nama" id="nama" value="{{ old('nama', $mahasiswa->nama) }}" class="mt-1 block w-full rounded-md border-stone-300 shadow-sm focus:border-stone-500 focus:ring focus:ring-stone-200 focus:ring-opacity-50">
                            @error('nama')
                        ...
                        <div>
                            ...
                            <select name="nidn" id="nidn" class="mt-1 block w-full rounded-md border-stone-300 shadow-sm focus:border-stone-500 focus:ring focus:ring-stone-200 focus:ring-opacity-50">
                                @foreach($dosen as $d)
