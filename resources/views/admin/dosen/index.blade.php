    ...
    <div class="flex min-h-screen bg-stone-100">
        ...
        <aside class="w-64 bg-slate-800 text-slate-100 flex flex-col shadow-xl">
            ...
            <nav class="flex-1 p-4 space-y-1">
                ...
                <a href="{{ route('dosen.index') }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-lg bg-slate-700 font-medium transition">
                    <span>👥 Dosen</span>
                </a>
        ...
        <main class="flex-1 p-8">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-stone-800">Data Dosen</h1>
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm border border-stone-200 mb-6">
                <form action="{{ route('dosen.index') }}" method="GET" class="flex flex-col md:flex-row md:items-center gap-3">
                    <div class="flex-1 relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau NIDN dosen..." class="w-full pl-9 pr-4 py-2 border border-stone-300 rounded-lg text-sm bg-stone-50 focus:bg-white focus:ring-slate-500 focus:border-slate-500 transition text-stone-800">
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if(request('search') || request('sort'))
                            <a href="{{ route('dosen.index') }}" class="px-4 py-2 bg-stone-200 hover:bg-stone-300 text-stone-700 font-semibold text-sm rounded-lg transition duration-150">
                                Reset
                            </a>
                        @endif
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md border border-stone-200 overflow-hidden">
                <div class="bg-slate-700 px-6 py-4 flex justify-between items-center text-white">
                    ...
                    <a href="{{ route('dosen.create') }}" class="bg-white text-stone-800 hover:bg-stone-100 transition text-xs font-bold px-4 py-2 rounded-lg border border-stone-200 flex items-center space-x-1 shadow-sm">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-stone-50 border-b border-stone-200 text-stone-600 text-xs font-bold uppercase tracking-wider">
                                ...
                                <th class="py-4 px-6 w-48">NIDN</th>
                        ...
                        <tbody class="divide-y divide-stone-100 text-sm text-stone-700">
                            @forelse($dosen as $index => $d)
                                <tr class="hover:bg-stone-50/50 transition">
                                    ...
                                    <td class="py-4 px-6 font-mono text-pink-600 font-bold tracking-wide">
                                        {{ $d->nidn }}
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-center whitespace-nowrap">
                                        <div class="inline-flex items-center justify-center space-x-2">
                                            ...
                                            <a href="{{ route('dosen.edit', $d->nidn) }}" class="p-1.5 bg-amber-500 text-white rounded hover:bg-amber-600 transition shadow-sm flex items-center justify-center w-7 h-7" title="Edit Data">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-8 px-6 text-center text-stone-400 italic">
                                        🔍 Tidak ditemukan data dosen yang cocok dengan pencarian.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
